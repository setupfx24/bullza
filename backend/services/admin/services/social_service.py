...
def _promo_demo_user_ids():
    """Sub-select of promotional/demo user ids — hidden from every admin view
    (client 2026-07-16). Master pools owned by such users must not appear."""
...
async def pamm_analytics(db: AsyncSession) -> dict:
    ...
    pamm_result = await db.execute(
        ...
        .where(MasterAccount.status == "approved", MasterAccount.master_type == "pamm",
               MasterAccount.user_id.notin_(_hide))
    ...
    # Sum all admin platform commissions (type=admin_commission from
    # credit_admin_fee). Exclude fees generated by promo/demo masters/investors
    # so the showcase pools don't inflate it (client 2026-07-16). NULL user_id
    # rows kept (can't attribute; notin_ would drop them).
    from sqlalchemy import or_ as _or
    fee_result = await db.execute(
        select(func.coalesce(func.sum(Transaction.amount), 0)).where(
            Transaction.type == "admin_commission",
            _or(Transaction.user_id.is_(None), Transaction.user_id.notin_(_hide)),
        )
    )
...
async def list_master_requests(page: int, per_page: int, db: AsyncSession) -> dict:
    query = select(MasterAccount).where(
        MasterAccount.status == "pending",
        MasterAccount.user_id.notin_(_promo_demo_user_ids()),
    )
