...
async def get_lp_status(db: AsyncSession) -> dict:
    ...
    if not lp_enabled:
        message = "CORECEN_LP_ENABLED is false — this platform is not configured to receive LP prices"
    elif last_batch_at is None:
        message = "Waiting for Corecen to push the first price batch (check the platform API URL + HMAC keys on the Corecen side)"
    elif connected:
