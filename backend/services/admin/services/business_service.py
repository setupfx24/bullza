...
def _promo_ids():
    """Sub-select of promotional/demo user ids — excluded from ALL admin
    listings (client 2026-07-16: "promotional wale account ka kuch bhi admin me
    na show ho"). Includes demo so showcase + practice accounts both stay
    hidden from the admin panel while remaining visible on the trader side."""
...
async def list_masters(
    ...
    *, master_type: str | None = None,
    ...
    # Hide masters owned by promo/demo showcase users (client 2026-07-16).
    base_filters = [MasterAccount.user_id.notin_(_promo_ids())]
    if master_type:
    ...
    count_stmt = select(func.count(MasterAccount.id)).where(*base_filters)
...
async def admin_commission_summary(
    *, master_type: str | None, db: AsyncSession,
    ...
    """
    # Lifetime total — exact. Exclude commission generated by promo/demo
    # masters/investors (client 2026-07-16). NULL user_id kept (unattributable).
    from sqlalchemy import or_ as _or
    total_q = await db.execute(
        select(func.coalesce(func.sum(Transaction.amount), 0))
        .where(Transaction.type == "admin_commission",
               _or(Transaction.user_id.is_(None), Transaction.user_id.notin_(_promo_ids())))
    )
    lifetime_total = float(total_q.scalar() or 0)

    # Per-master breakdown — hide promo/demo-owned masters.
    filters = [MasterAccount.user_id.notin_(_promo_ids())]
    if master_type:
        normalized = master_type.strip().lower()
        if normalized in ("signal_provider", "pamm", "mamm"):
...
async def list_master_allocations(
    ...
    result = await db.execute(
        ...
        # Hide promo/demo investors (client 2026-07-16).
        .where(InvestorAllocation.master_id == master_id,
               InvestorAllocation.investor_user_id.notin_(_promo_ids()))
