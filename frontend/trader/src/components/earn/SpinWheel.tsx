...
const SLICE_COLOURS = ['#E85D3D', '#9c7a30', '#c69a37', '#7a5e26', '#e0bc63', '#8c6c2b', '#b58a35'];
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      <div className="relative w-[300px] h-[300px] sm:w-[360px] sm:h-[360px]">
        ...
        <div
          ref={wheelRef}
          className="absolute inset-0 rounded-full border-4 border-[#E85D3D]/45 overflow-hidden"
          style={{
            background: conicGradient,
            transform: `rotate(${angle}deg)`,
            transition: spinning ? 'transform 3.4s cubic-bezier(0.17, 0.67, 0.30, 0.99)' : 'none',
            boxShadow: '0 0 32px rgba(232, 93, 61,0.18), inset 0 0 32px rgba(0,0,0,0.4)',
          }}
        >
          {prizes.map((p, i) => {
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div
            className="w-16 h-16 rounded-full bg-bg-base border-2 border-[#E85D3D] flex items-center justify-center"
            style={{ boxShadow: '0 0 18px rgba(232, 93, 61,0.5)' }}
          >
            <Sparkles size={22} className="text-[#E85D3D]" />
          </div>
        </div>
      </div>
      ...
      <button
        ...
        className="inline-flex items-center gap-2 px-7 py-3 rounded-lg text-sm font-bold bg-[#E85D3D] text-bg-base hover:brightness-110 disabled:opacity-60 transition-all"
      >
        {spinning ? (
          <>
