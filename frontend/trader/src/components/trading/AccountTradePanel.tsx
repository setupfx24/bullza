    ...
    <div className="rounded-xl border border-border-primary bg-bg-base overflow-hidden mt-3 trade-panel-enter">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-border-primary bg-bg-secondary">
        <div className="flex items-center gap-2.5">
          <div className="w-2 h-2 rounded-full" style={{ background: marketStatus.isOpen ? '#E12019' : '#ef5350' }} />
          ...
          <div className="p-3 space-y-3">
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  {isConnected ? (
                    <span className="flex items-center gap-1" title="Live prices">
                      <Wifi size={10} className="text-[#E12019]" />
                      <span className="w-1.5 h-1.5 rounded-full bg-[#E12019] animate-pulse" />
                    </span>
                  ) : (
                  ...
                  {displaySymbols.map((sym) => {
                    ...
                    return (
                      <button
                        ...
                        style={{ borderLeft: sym === selectedSymbol ? '2px solid #E12019' : '2px solid transparent' }}
                      >
                        ...
                        {t && (
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-[10px] font-mono text-[#E12019]">{t.ask.toFixed(d)}</span>
                          </div>
                        )}
                      </button>
                    );
            ...
            <div className="flex border-b border-border-primary">
              {(['market', 'pending'] as const).map((t) => (
                <button
                  ...
                  style={{
                    borderBottom: orderTab === t ? '2px solid var(--accent, #E12019)' : '2px solid transparent',
            ...
            {tick && (
              <div className="flex items-center justify-between px-1">
                ...
                <div className="text-center">
                  <div className="text-xs font-mono font-semibold text-[#E12019]">{tick.ask.toFixed(digits)}</div>
            ...
            <div className="flex items-center gap-4 border-t border-border-primary pt-2.5">
              ...
              <label className="flex items-center gap-1.5 cursor-pointer">
                <div
                  ...
                  className="w-8 h-[18px] rounded-full relative transition-colors cursor-pointer"
                  style={{ background: tpEnabled ? '#E12019' : 'var(--bg-secondary)' }}
                >
            ...
            {tpEnabled && (
              <div>
                <span className="text-[10px] font-semibold uppercase tracking-wider text-[#E12019] mb-1 block">Take Profit</span>
                <input
                  type="number"
                  value={takeProfit}
                  ...
                  className="w-full text-sm font-mono py-2 px-3 rounded-lg focus:outline-none"
                  style={{ background: 'var(--bg-secondary)', border: '1px solid rgba(225, 32, 25,0.25)', color: '#E12019' }}
                />
              </div>
            )}
            ...
            <div className="rounded-xl p-2.5 space-y-1.5 bg-bg-secondary" style={{ border: `1px solid ${!hasEnoughMargin ? 'rgba(239,83,80,0.3)' : 'var(--border-primary)'}` }}>
              {[
                ...
                { label: 'Free Margin', value: fmtAccountMoney(freeMargin, isCentAccount(account)), color: !hasEnoughMargin ? '#ef5350' : '#E12019' },
                { label: 'Feed', value: isConnected ? '● Connected' : '○ Disconnected', color: isConnected ? '#E12019' : '#f57c00' },
              ].map((row) => (
            ...
            <button
              ...
              style={{
                ...
                boxShadow: side === 'buy' ? '0 4px 20px rgba(225, 32, 25,0.2)' : '0 4px 20px rgba(239,83,80,0.2)',
              }}
            >
        ...
        <div className="lg:w-[340px] shrink-0 border-t border-border-primary">
          ...
          {posLoading && positions.length === 0 ? (
            <div className="flex items-center justify-center py-6">
              <div className="w-4 h-4 border-2 border-[#E12019] border-t-transparent rounded-full animate-spin" />
            </div>
          ) : positions.length === 0 ? (
            ...
            <div className="overflow-y-auto" style={{ maxHeight: '200px' }}>
              {positions.map((pos) => {
                ...
                const livePnl = cp > 0
                  ...
                  : pos.profit;
                const pnlColor = livePnl >= 0 ? '#E12019' : '#ef5350';
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5">
                        <span
                          ...
                          style={{
                            background: pos.side === 'buy' ? 'rgba(225, 32, 25,0.12)' : 'rgba(239,83,80,0.12)',
          ...
          <div className="px-3 py-2 space-y-1 border-t border-border-primary bg-bg-secondary">
            {[
              ...
              { label: 'Free Margin', value: fmtAccountMoney(account.free_margin, isCentAccount(account)), color: '#E12019' },
            ].map((r) => (
