    ...
    <div className="rounded-xl border border-border-primary bg-bg-base overflow-hidden mt-3 trade-panel-enter">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 border-b border-border-primary bg-bg-secondary">
        <div className="flex items-center gap-2.5">
          <div className="w-2 h-2 rounded-full" style={{ background: marketStatus.isOpen ? '#E85D3D' : '#ef5350' }} />
          ...
          <div className="p-3 space-y-3">
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  {isConnected ? (
                    <span className="flex items-center gap-1" title="Live prices">
                      <Wifi size={10} className="text-[#E85D3D]" />
                      <span className="w-1.5 h-1.5 rounded-full bg-[#E85D3D] animate-pulse" />
                    </span>
                  ) : (
                  ...
                  {displaySymbols.map((sym) => {
                    ...
                    return (
                      <button
                        ...
                        style={{ borderLeft: sym === selectedSymbol ? '2px solid #E85D3D' : '2px solid transparent' }}
                      >
                        ...
                        {t && (
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-[10px] font-mono text-[#E85D3D]">{t.ask.toFixed(d)}</span>
                          </div>
                        )}
                      </button>
                    );
            ...
            <div className="flex border-b border-border-primary">
              {(['market', 'pending'] as const).map((t) => (
                <button
                  ...
                  style={{
                    borderBottom: orderTab === t ? '2px solid var(--accent, #E85D3D)' : '2px solid transparent',
            ...
            <div className="grid grid-cols-2 gap-1.5">
              ...
              <button
                ...
                className="py-3 rounded-lg text-center transition-all duration-150 active:scale-[0.97]"
                style={{
                  background: side === 'buy' ? '#E85D3D' : 'rgba(232, 93, 61,0.12)',
                  color: side === 'buy' ? '#000' : '#E85D3D',
                }}
              >
            ...
            {tick && (
              <div className="flex items-center justify-between px-1">
                ...
                <div className="text-center">
                  <div className="text-xs font-mono font-semibold text-[#E85D3D]">{tick.ask.toFixed(digits)}</div>
            ...
            {tpEnabled && (
              <div>
                <span className="text-[10px] font-semibold uppercase tracking-wider text-[#E85D3D] mb-1 block">Take Profit</span>
                <input
                  type="number"
                  value={takeProfit}
                  ...
                  className="w-full text-sm font-mono py-2 px-3 rounded-lg focus:outline-none"
                  style={{ background: 'var(--bg-secondary)', border: '1px solid rgba(232, 93, 61,0.25)', color: '#E85D3D' }}
                />
              </div>
            )}
            ...
            <div className="rounded-xl p-2.5 space-y-1.5 bg-bg-secondary" style={{ border: `1px solid ${!hasEnoughMargin ? 'rgba(239,83,80,0.3)' : 'var(--border-primary)'}` }}>
              {[
                ...
                { label: 'Free Margin', value: fmtAccountMoney(freeMargin, isCentAccount(account)), color: !hasEnoughMargin ? '#ef5350' : '#E85D3D' },
        ...
        <div className="lg:w-[340px] shrink-0 border-t border-border-primary">
          ...
          {posLoading && positions.length === 0 ? (
            <div className="flex items-center justify-center py-6">
              <div className="w-4 h-4 border-2 border-[#E85D3D] border-t-transparent rounded-full animate-spin" />
            </div>
          ) : positions.length === 0 ? (
            ...
            <div className="overflow-y-auto" style={{ maxHeight: '200px' }}>
              {positions.map((pos) => {
                ...
                const livePnl = cp > 0
                  ...
                  : pos.profit;
                const pnlColor = livePnl >= 0 ? '#E85D3D' : '#ef5350';
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center gap-1.5">
                        <span
                          ...
                          style={{
                            background: pos.side === 'buy' ? 'rgba(232, 93, 61,0.12)' : 'rgba(239,83,80,0.12)',
                            color: pos.side === 'buy' ? '#E85D3D' : '#ef5350',
                          }}
                        >
                          {pos.side}
                        </span>
          ...
          <div className="px-3 py-2 space-y-1 border-t border-border-primary bg-bg-secondary">
            {[
              ...
              { label: 'Free Margin', value: fmtAccountMoney(account.free_margin, isCentAccount(account)), color: '#E85D3D' },
            ].map((r) => (
