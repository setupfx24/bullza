        ...
        <div className="flex flex-col items-end shrink-0">
          <div className="flex items-center gap-1">
            <span
              ...
              style={{ color: marketStatus.isOpen ? '#E12019' : '#f57c00' }}
            >
              {marketStatus.isOpen ? 'OPEN' : 'CLOSED'}
            </span>
            {isConnected ? (
              <Wifi size={isTradingTerminal ? 11 : 12} className="text-[#E12019]" />
            ) : (
        ...
        >
            ...
            {(['market', 'pending'] as const).map((t) => (
              <button
                ...
                style={{
                  ...
                  borderBottom:
                    orderTab === t
                      ? `2px solid ${isTradingTerminal ? '#2962FF' : '#E12019'}`
          ...
          <div className={clsx('grid grid-cols-2', isTradingTerminal ? 'gap-1.5' : 'gap-2')}>
             ...
             <button
                ...
                className={clsx(obPad, 'rounded-lg flex flex-col items-center justify-center transition-all duration-150 active:scale-[0.98]')}
                style={{
                  background: side === 'buy' ? 'rgba(225, 32, 25,0.15)' : 'var(--bg-secondary)',
          ...
          {tpEnabled && (
            <div className="pt-2">
              <span className="text-[10px] font-bold uppercase tracking-wider text-[#E12019] mb-1.5 block">Take Profit</span>
              <input
                type="number"
                value={takeProfit}
                ...
                className="w-full text-sm font-mono py-2.5 px-3 rounded-lg focus:outline-none bg-bg-secondary border border-[#E12019]/30 text-[#E12019]"
              />
            </div>
          )}
            ...
            <>
              ...
              <div className="rounded-xl p-3 space-y-2 bg-bg-secondary border border-border-primary">
                {[
                  ...
                  { label: 'Free Margin', value: fmtAccountMoney(freeMargin, isCentAccount(activeAccount)), color: !hasEnoughMargin ? '#ef5350' : '#E12019' },
        ...
        {isTradingTerminal ? (
          <div className="shrink-0 border-t border-border-primary bg-bg-secondary px-2 pt-2 pb-2 space-y-1.5">
            ...
            <div className="flex items-center justify-between gap-1 px-1 text-[9px] text-text-tertiary">
              ...
              <span className={clsx('shrink-0 font-mono', hasEnoughMargin ? 'text-[#E12019]' : 'text-[#ef5350]')}>
                Free {fmtAccountMoney(freeMargin, isCentAccount(activeAccount))}
              </span>
              <span
                className={clsx('shrink-0 font-mono', isConnected ? 'text-[#E12019]' : 'text-[#f57c00]')}
            ...
            <button
              ...
              style={{
                background: side === 'buy' ? '#55A630' : '#ef5350',
                color: '#fff',
                boxShadow: side === 'buy' ? '0 2px 12px rgba(225, 32, 25,0.2)' : '0 2px 12px rgba(239,83,80,0.2)',
              }}
            >
              {`${side === 'buy' ? 'Buy' : 'Sell'} ${selectedSymbol}`}
            </button>
            ...
            {presets.map((v) => (
              <button
                ...
                className={clsx(
                  ...
                  v === account.leverage
                    ? 'bg-[#E12019]/15 text-[#E12019] font-bold'
