      ...
      <div className="max-w-xl mx-auto w-full px-4 py-6 space-y-5">
        ...
        {step === 'form' && sel && (
          <>
            ...
            <button
              ...
              className="w-full py-3 rounded-xl bg-[#E12019] text-white font-bold disabled:opacity-50"
            >Deposit</button>
          </>
        )}
        ...
        {step === 'qr' && sel && (
          <>
            ...
            <div className="rounded-xl border border-border-primary bg-bg-secondary p-4 space-y-3 text-center">
              ...
              <div className="rounded-lg bg-amber-500/10 border border-amber-500/30 px-3 py-2 text-left">
                ...
                <div className="flex items-center justify-between gap-2">
                  ...
                  <button type="button" onClick={() => copyText(reference, 'Reference')} className="text-[10px] font-semibold text-[#E12019]">Copy</button>
                </div>
              </div>
              ...
              {sel.upi_id ? <p className="text-xs">UPI: <span className="font-mono">{sel.upi_id}</span> <button type="button" onClick={() => copyText(sel.upi_id!, 'UPI')} className="text-[10px] text-[#E12019]">Copy</button></p> : null}
            ...
            <button type="button" disabled={submitting || !utr.trim()} onClick={() => void confirmPayment()} className="w-full py-3 rounded-xl bg-[#E12019] text-white font-bold disabled:opacity-50">{submitting ? 'Submitting…' : 'Confirm Payment'}</button>
          </>
        )}
      </div>
      ...
      {noticeOpen && sel && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md rounded-2xl border border-border-primary bg-bg-primary p-5 space-y-3">
            ...
            <button type="button" onClick={() => { setNoticeOpen(false); setStep('form'); }} className="w-full py-2.5 rounded-xl bg-[#E12019] text-white font-bold">Accept and Continue</button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md rounded-2xl border border-border-primary bg-bg-primary p-5 space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <button type="button" onClick={() => { setConfirmOpen(false); setStep('qr'); }} className="flex-1 py-2.5 rounded-xl bg-[#E12019] text-white font-bold">Confirm</button>
            </div>
          </div>
        </div>
      )}
